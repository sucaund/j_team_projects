	...
	<table class="table" >
		...
		<c:choose>
			<c:when test='${listPoint == null or empty listPoint}'>
				<td colspan="4">
					<p align="center">
						<b><span style=''>포인트 사용내역이 없습니다.</span></b>
					</p>
				</td>
			</c:when>
...
<div style="display: grid; place-items: center;">
	...
	<c:if test="${page.startPage > page.pageBlock }">
		<li class="page-item"><a class="page-link" href="hsListUsePoint?currentPage=${page.startPage-page.pageBlock}&m_number=${memberData.m_number}">[이전]</a>
	</c:if>
	<c:forEach var="i" begin="${page.startPage}" end="${page.endPage}">
		<li class="page-item"><a class="page-link" href="hsListUsePoint?currentPage=${i}&m_number=${memberData.m_number}">${i}</a>
	</c:forEach>
	<c:if test="${page.endPage < page.totalPage }">
		<li class="page-item"><a class="page-link" href="hsListUsePoint?currentPage=${page.startPage+page.pageBlock}&m_number=${memberData.m_number}">[다음]</a>
	</c:if>	
		</ul>
			</div>
		</div>
