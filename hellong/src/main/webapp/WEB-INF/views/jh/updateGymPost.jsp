...
<div class="container">
    ...
    <c:forEach var="gym" items="${gymDetail}">
	    <c:forEach var="gymBoard" items="${gymBoardDetail}">   
            ...
            <div class="form-group">
                <label>제목</label>
                <textarea class="form-control" name="gb_title" rows="1" required>${gymBoard.gb_title}</textarea>
            </div>
            <div class="form-group">
                <label>헬스장 기본소개</label>
                <textarea class="form-control" name="gb_ment" rows="10" required>${gymBoard.gb_ment}</textarea>
            </div>
            <div class="form-group">
                <label>위치</label>
            ...
            <div class="form-group">
                <label>운동기구</label>
                <textarea class="form-control" name="gb_machine" rows="3">${gymBoard.gb_machine}</textarea>
            </div>
            <div class="form-group">
                <label>편의시설</label>
                <textarea class="form-control" name="gb_amen" rows="2">${gymBoard.gb_amen}</textarea>
            </div>
            <div class="form-group">
                <label>운영시간</label>
                <textarea class="form-control" name="gb_time" rows="2">${gymBoard.gb_time}</textarea>
            </div>
            ...
            <div class="form-group">
                <label>동영상 URL</label>
                <textarea class="form-control" name="gb_video" rows="1">${gymBoard.gb_video}</textarea>
            </div>
            <div class="form-group">
                <label>작성날짜</label>
            ...
            <div class="button-group">
                <button type="submit" class="btn btn-primary" onclick="handleFormSubmission()">저장</button>
                <button type="button" class="btn btn-danger delete-btn" onclick="if(confirm('정말 삭제하시겠습니까?')) location.href='deleteGymPost?g_id=${gymBoard.g_id}&m_number=${gym.m_number}'">삭제</button>
